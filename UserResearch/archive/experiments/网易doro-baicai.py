...
class TypeMarkerDialog:
    ...
    def _parse_columns_for_questions(self, columns):
        """根据新的问卷格式解析列名，提取题号和题干"""
        questions_data = defaultdict(lambda: {'stem': '', 'all_cols': []})
        q_num_pattern = re.compile(r'^Q(\d+)\.')

        for col in columns:
            match = q_num_pattern.match(col)
            if not match:
                continue

            q_num = int(match.group(1))
            questions_data[q_num]['all_cols'].append(col)
            ...
            # ******** 代码修改结束 ********
            stem = stem_part.replace(f'Q{q_num}.', '', 1).strip()
...
def advanced_split(col_name):
    """
    Splits a column name into (stem_part, option_part).
    Splits at the first ':' found after the first '?' or '？'.
    Falls back to rsplit at the last ':' if the primary rule doesn't apply.
    """
    q_mark_pos = -1
    # Find the first occurrence of either English or Chinese question mark
    try:
        q_mark_pos = col_name.index('?')
    except ValueError:
        try:
            q_mark_pos = col_name.index('？')
        except ValueError:
            pass # No question mark found

    if q_mark_pos != -1:
        # Find the first colon after the question mark
        try:
            # Start searching for ':' from the character right after the '?'
            colon_pos = col_name.index(':', q_mark_pos + 1)
            stem_part = col_name[:colon_pos]
            option_part = col_name[colon_pos + 1:]
            return stem_part, option_part
        except ValueError:
            pass # No colon found after the question mark

    # Fallback logic: split at the last colon
    if ':' in col_name:
        return col_name.rsplit(':', 1)
    else:
        return col_name, "" # No colon at all

def extract_qnum(col_name):
    """从列名中提取题号 (Q后面的数字)"""
    col_str = str(col_name).strip()
    # 优先匹配 "Qx." 格式
    match = re.match(r'^\s*Q(\d+)\.', col_str)
    if match:
...
class SurveyProcessorApp:
    def __init__(self, master):
        ...
        self.player_type_combobox.grid(row=1, column=1, columnspan=2, padx=5, pady=5, sticky="ew")
    ...
    def process_survey_data(self, df_input, player_type_column_name, min_duration_value, question_types_dict, output_excel_path, run_statistical_tests):
        ...
        try:
            ...
            raise ValueError("未找到'答题时长（秒）'列，请检查数据列名，如果时间列不叫‘答题时长（秒）’请修改该列名称")
        ...
        for q_type, q_nums in QUESTION_TYPES.items():
            for q_num in q_nums:
                ...
                if q_type == '单选':
                    ...
                    if q_cols:
                        all_options = original_df[q_cols[0]].dropna().unique().tolist()
        ...
        for q in QUESTION_TYPES.get("多选", []):
            ...
            option_to_col = {extract_option(c, q_num_str): c for c in q_cols}
