...
# 新增 Repeated Measures ANOVA 的导入
from statsmodels.stats.anova import AnovaRM
# 新增配对检验的导入
from scipy.stats import ttest_rel, wilcoxon, friedmanchisquare
from survey_tools.utils.outline_parser import parse_outline_for_platform, outline_to_q_num_type


# ========== 新增：手动标记题型对话框 ==========
class TypeMarkerDialog:
    """一个用于手动标记问卷题目类型的对话框"""
    def __init__(self, parent, columns, prefilled_types=None):
        ...
        self.result = None
        self.prefilled_types = prefilled_types or {}
        self.comboboxes = {}
        self.question_info = self._parse_columns_for_questions(columns)
        # 预先根据列名中的提示词（如【单选】/【多选】/【评分】等）推断题型，供默认选择使用
        self.inferred_types = {
            q_num: self._infer_type_from_columns(info)
        ...
        for i, (q_num, info) in enumerate(sorted(self.question_info.items())):
            ...
            # 优先使用大纲预标记；若无则回退到列名启发式推断
            prefilled_display_type = self.prefilled_types.get(q_num)
            inferred_display_type = self.inferred_types.get(q_num)
            if prefilled_display_type in q_types:
                combo.set(prefilled_display_type)
            elif inferred_display_type in q_types:
    ...
    def _infer_type_from_columns(self, info):
        """
        根据该题关联的所有列名文本，尝试自动推断题型。

        仅作为“默认选择”使用，优先级低于用户手动修改。
        支持根据常见标记自动识别：
        - 【单选】、单选题
        - 【多选】、多选题、限选X个
        - 【评分】、评分题
        ...
        # 将所有相关列名拼接在一起，去掉空格，便于关键词匹配
        joined = '｜'.join(str(c) for c in cols)
        text = joined.replace(' ', '')

        # 矩阵题优先判断，避免“矩阵单选题”被误判为普通单选题
        if '矩阵单选' in text or '【矩阵单选' in text:
            return '矩阵单选题'
        if '矩阵评分' in text or '【矩阵评分' in text:
            return '矩阵评分题'

        # 多选题：常见标记有【多选】、【多选题】、“多选，限选X个”等
        if '【多选题' in text or '【多选】' in text or '多选题' in text:
            return '多选题'
        if '多选' in text and '限选' in text:
            return '多选题'

        # 单选题：标记【单选】或“单选题”
        if '【单选】' in text or '【单选题' in text or '单选题' in text:
            return '单选题'

        # 评分题：标记【评分】、“评分题”等
        if '【评分】' in text or '评分题' in text or '打分题' in text:
            return '评分题'

        # 无法可靠判断时，返回 None，交给用户手动选择
        return None
    ...
    def _parse_columns_for_questions(self, columns):
        ...
        - 兼容多种题号格式，例如：
          ...
          - Q1（单选）
          - Q1_1, Q1_2（多选题的子项列）
        - 实际的题号提取统一使用 extract_qnum，避免只匹配 'Q1.' 这类格式。
        """
        ...
        for col in columns:
            ...
            # 使用全局的 extract_qnum 来解析题号，支持 Q1、Q1_1、Q1（单选）等格式
...
class CustomSegmentDialog:
    def __init__(self, parent, file_path):
        ...
        try:
            ...
            self.df_sample = pd.read_excel(file_path)
...
def calculate_rank_biserial_correlation(group1_data, group2_data):
    ...
    if n1 == 0 or n2 == 0:
        return np.nan
    try:
        u_stat, _ = stats.mannwhitneyu(group1_data, group2_data, alternative='two-sided', method='auto')
        rb = 1 - (2 * u_stat) / (n1 * n2)
        rb = max(-1.0, min(1.0, rb))
        return rb
    except Exception as e:
        print(f"Warning: Could not calculate rank-biserial correlation: {e}")
        return np.nan
...
def calculate_paired_rank_biserial_correlation(data1, data2):
    ...
    if len(data1) != len(data2):
        print("Error: Paired rank biserial correlation requires equal length data.")
        return np.nan
    ...
    W_pos = np.sum(ranks[diff_nonzero > 0])
    W_neg = np.sum(ranks[diff_nonzero < 0])
...
def calculate_cohens_g_mcnemar(contingency_table_2x2):
    try:
        if isinstance(contingency_table_2x2, pd.DataFrame):
            ...
            table_arr = np.array(contingency_table_2x2)

        if table_arr.shape != (2, 2):
            print(f"Warning: calculate_cohens_g_mcnemar received a non-2x2 table. Shape: {table_arr.shape}")
            return np.nan
        ...
        print(f"Error calculating Cohen's g for McNemar: {e}")
        return np.nan
...
def calculate_cohens_d_paired(group1_data, group2_data):
    ...
    if len(data1) != len(data2):
        print("Warning: Paired Cohen's d requires equal length paired data.")
        return np.nan
    diff = data1 - data2
    n_diff = len(diff)
    if n_diff <= 1:
...
def advanced_split(col_name):
    """
    将列名拆分为 (stem_part, option_part)，支持中英文标点：
    - 优先在第一个问号（`?` 或 `？`）之后寻找第一个冒号（英文 `:` 或中文 `：`）；
    - 如果失败，则在整串中按最后一个冒号（中英文皆可）进行切分；
    - 如果完全没有冒号，则返回 (整串, 空字符串)。
    """
    if not isinstance(col_name, str):
        col_name = str(col_name)

    # 为了便于后续处理，先保留原串，再构造一个把中文冒号统一替换为英文冒号的副本
    original = col_name
...
def extract_qnum(col_name):
    ...
    # 优先匹配常见的 "Qx", "Qx.", "Qx_1" 等格式
    match = re.match(r'^\s*Q(\d+)(?:[.\s_\-]|$)', col_str)
    if match:
        return match.group(1)
    # 兼容旧格式作为后备
    col_str = re.sub(r'（.*?）', '', col_str)
    col_str = re.sub(r'\((.*?)\)', '', col_str)
    patterns = [r'^(\d+)[、.)]', r'\bQ?(\d+)[\s_\-]', r'(?:问题|题目|Question)\s*(\d+)']
...
def get_question_columns(df_like, q_num_str, include_colon=None):
    """按题号稳健取列，兼容 Qx. / x、 / x(选项) 等格式。

    Args:
        df_like: DataFrame（使用其 columns）
        q_num_str: 题号字符串
        include_colon:
            - True: 仅保留含冒号（: / ：）列（常用于多选/矩阵子列）
            - False: 仅保留不含冒号列（常用于单选/评分主列）
            - None: 不按冒号过滤
    """
    cols = []
    for c in df_like.columns:
        c_str = str(c).strip()
        if extract_qnum(c_str) != str(q_num_str):
            continue
        has_colon = (':' in c_str) or ('：' in c_str)
        if include_colon is True and not has_colon:
            continue
        if include_colon is False and has_colon:
            continue
        cols.append(c)
    return cols

def extract_option(col_name, question_num):
    """根据新的格式 'Q<num>.<stem>:<option>' 提取选项。"""
    col_str = str(col_name).strip()
    # ******** 代码修改处 (2/6) ********
    # 使用新的切分逻辑
...
def normalize_multi_option_label(col_name: str, q_num_str: str | None = None) -> str:
    """稳健提取多选/矩阵子列的选项文本。
    ...
    # 2) 对“限选N个”题的首列，优先提取末尾括号中的首个选项
    if re.search(r"(?:限选|最多选|最多可选)\s*\d+\s*(?:个|项)", s):
        m_tail = re.search(r"[)）]\s*[（(]\s*(.+?)\s*[)）]\s*$", s)
    ...
    if q_num_str:
        s = re.sub(
            rf"^\s*(?:Q)?{re.escape(str(q_num_str))}\s*[、.。)\]）(（]?\s*",
            "",
            s,
        ).strip()
    else:
        s = re.sub(r"^\s*(?:Q)?\d+\s*[、.。)\]）(（]?\s*", "", s).strip()

    # 4) 若整体被括号包裹，去壳
    m_wrap = re.match(r"^[（(]\s*(.+?)\s*[)）]\s*$", s)
    if m_wrap and m_wrap.group(1).strip():
        s = m_wrap.group(1).strip()

    # 5) 纯占位内容视为无效选项
    if re.fullmatch(r"[\s_＿()（）:：-]+", s):
        return ""
    return s
...
def get_question_stem(original_df, question_number):
    ...
    # 查找所有与该题号相关的列（兼容 Qx. / x、 / x(）等）
    q_cols = get_question_columns(original_df, q_num_str, include_colon=None)
...
def convert_matrix_single_questions(df, matrix_single_modules):
    ...
    for q_num in matrix_single_modules:
        q_num_str = str(q_num)
        # 查找所有属于这个矩阵题的列 (格式: Qx. ... : ...)
        cols_for_q = get_question_columns(df, q_num_str, include_colon=True)
...
class ExcelMergerApp:
    ...
    def start_merge_thread(self):
        ...
        merge_thread = threading.Thread(target=self._run_merge_in_thread,
                                        args=(file1, file2, col1, col2, output_file_path))
        merge_thread.start()

    def _run_merge_in_thread(self, file1, file2, col1, col2, output_file_path):
        try:
            original_df1 = pd.read_excel(file1, dtype={col1: str})
            original_df2 = pd.read_excel(file2, dtype={col2: str})
...
class SurveyProcessorApp:
    def __init__(self, master):
        ...
        self.outline_path = tk.StringVar()
        self.outline_platform_var = tk.StringVar(value="问卷星")
        ...
        tk.Label(main_frame, text="可选：问卷大纲:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.outline_entry = tk.Entry(main_frame, textvariable=self.outline_path, state="readonly")
        self.outline_entry.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        outline_ctrl = ttk.Frame(main_frame)
        outline_ctrl.grid(row=1, column=2, padx=5, pady=5, sticky="e")
        ttk.Combobox(
            outline_ctrl,
            textvariable=self.outline_platform_var,
            values=["问卷星", "腾讯问卷"],
            state="readonly",
            width=8,
        ).pack(side="left", padx=(0, 4))
        tk.Button(outline_ctrl, text="浏览...", command=self.browse_outline_file).pack(side="left")

        # 初始化自定义分群字典
        self.custom_segments = {}

        tk.Label(main_frame, text="选择玩家类型列:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.player_type_combobox = ttk.Combobox(main_frame, textvariable=self.player_type_col, state="readonly")
        self.player_type_combobox.grid(row=2, column=1, padx=5, pady=5, sticky="ew")
        # 新增自定义分群按钮
        self.btn_custom_seg = tk.Button(main_frame, text="或：自定义分群", command=self.launch_custom_segments, state="disabled")
        self.btn_custom_seg.grid(row=2, column=2, padx=5, pady=5, sticky="e")

        tk.Label(main_frame, text="最低完成时间 (秒):").grid(row=3, column=0, padx=5, pady=5, sticky="w")
        self.min_duration_entry = tk.Entry(main_frame, textvariable=self.min_duration_var, width=10)
        self.min_duration_entry.grid(row=3, column=1, padx=5, pady=5, sticky="w")

        # ******** 代码修改处：新增复选框控件 ********
        self.run_stats_checkbutton = tk.Checkbutton(main_frame, text="生成统计检验结果 (若不勾选，则不输出显著性差异分析页)", variable=self.run_stats_test_var)
        self.run_stats_checkbutton.grid(row=4, column=0, columnspan=3, padx=5, pady=5, sticky="w")
        
        # ******** 代码修改处：调整后续控件的行号 ********
        self.type_marker_button = tk.Button(main_frame, text="手动标记题型", command=self.launch_type_marker, state="disabled")
        self.type_marker_button.grid(row=5, column=0, columnspan=3, pady=10)
        ...
        self.status_label.grid(row=7, column=0, columnspan=3, pady=5, sticky="ew")

        self.progress_bar = ttk.Progressbar(main_frame, orient="horizontal", mode="indeterminate")
        self.progress_bar.grid(row=8, column=0, columnspan=3, pady=5, sticky="ew")
    ...
    def browse_outline_file(self):
        outline_path = filedialog.askopenfilename(
            filetypes=[("Outline files", "*.docx *.txt"), ("All files", "*.*")]
        )
        if not outline_path:
            return
        self.outline_path.set(outline_path)
        try:
            with open(outline_path, "rb") as f:
                data = f.read()
            platform = "wjx" if self.outline_platform_var.get() == "问卷星" else "tencent"
            outline = parse_outline_for_platform(data, os.path.basename(outline_path), platform)
            raw_map = outline_to_q_num_type(outline)
            # 将内部类型映射到手动标记弹窗显示类型
            display_map = {}
            for q_num, t in raw_map.items():
                if t in ("单选",):
                    display_map[int(q_num)] = "单选题"
                elif t in ("多选",):
                    display_map[int(q_num)] = "多选题"
                elif t in ("评分", "NPS"):
                    display_map[int(q_num)] = "评分题"
                elif t in ("矩阵单选", "矩阵"):
                    display_map[int(q_num)] = "矩阵单选题"
                elif t in ("矩阵评分",):
                    display_map[int(q_num)] = "矩阵评分题"
                elif t in ("填空", "忽略"):
                    display_map[int(q_num)] = "填空题"
            self.outline_q_type_map = display_map
            self.status_label.config(
                text=f"大纲解析成功：{os.path.basename(outline_path)}，已识别 {len(display_map)} 道题型预标记。",
                foreground="green",
            )
        except Exception as e:
            self.outline_q_type_map = {}
            messagebox.showwarning("大纲解析失败", f"无法解析大纲，已回退为仅列名自动推断。\n{e}")
    ...
    def launch_type_marker(self):
        ...
        try:
            df_header = pd.read_excel(self.file_path.get(), nrows=0)
            columns = df_header.columns.tolist()
        except Exception as e:
            messagebox.showerror("文件读取错误", f"无法读取文件列名: {e}")
            return

        dialog = TypeMarkerDialog(self.master, columns, prefilled_types=self.outline_q_type_map)
    ...
    def launch_custom_segments(self):
        ...
        dialog = CustomSegmentDialog(self.master, f_path)
        self.master.wait_window(dialog.top)
    ...
    def start_analysis_thread(self):
        ...
        analysis_thread = threading.Thread(target=self._run_analysis_in_thread,
                                           args=(input_file, player_col, min_dur, self.QUESTION_TYPES, output_file_path, run_stats))
        analysis_thread.start()

    # ******** 代码修改处：_run_analysis_in_thread 接收新参数 ********
    def _run_analysis_in_thread(self, input_file, player_col, min_dur, question_types, output_file_path, run_stats):
        try:
    ...
    def process_survey_data(self, df_input, player_type_column_name, min_duration_value, question_types_dict, output_excel_path, run_statistical_tests):
        ...
        for q_type, q_nums in QUESTION_TYPES.items():
            for q_num in q_nums:
                ...
                if q_type == '单选':
                    # 先按“新格式”匹配 (Q<num>.题干)
                    q_cols = get_question_columns(original_df, q_num_str, include_colon=False)
                    ...
                    # 新格式：Q<num>.题干:选项
                    q_cols = get_question_columns(original_df, q_num_str, include_colon=True)
                    if q_cols:
                        ordered_options = [normalize_multi_option_label(c, q_num_str) for c in q_cols]
                        ordered_options = [opt for opt in ordered_options if str(opt).strip()]
                        ...
                        if fallback_cols:
                            ordered_options = [normalize_multi_option_label(c, q_num_str) for c in fallback_cols]
                            ordered_options = [opt for opt in ordered_options if str(opt).strip()]
                            self.sorted_question_options[('多选', q_num)] = list(dict.fromkeys(ordered_options))
                elif q_type == '矩阵评分' or q_type == '矩阵单选':
                    matrix_cols = get_question_columns(original_df, q_num_str, include_colon=True)
        ...
        for q in QUESTION_TYPES.get("单选", []):
            q_num_str = str(q)
            # 先尝试新格式 (Q<num>.题干)
            q_cols = get_question_columns(df, q_num_str, include_colon=False)
        ...
        for q in QUESTION_TYPES.get("评分", []):
            ...
            if q_cols:
                ...
                print(f"警告：评分题{q}未找到有效数值列（题号匹配 {q}），已跳过。")
                continue
        ...
        for q in QUESTION_TYPES.get("多选", []):
            q_num_str = str(q)
            # 新格式：Q<num>.题干:选项
            q_cols = get_question_columns(df, q_num_str, include_colon=True)
            if not q_cols:
                # 兼容旧格式 / 下划线格式：所有题号为 q 的列都视为选项列
                q_cols = get_question_columns(df, q_num_str, include_colon=None)
            ...
            option_to_col = {}
            for c in q_cols:
                opt_name = normalize_multi_option_label(c, q_num_str)
                if not str(opt_name).strip():
                    continue
                option_to_col[opt_name] = c

            if not option_to_col:
                print(f"警告：多选题{q}无有效选项列，跳过处理")
                continue

            # 对齐手动/预填充顺序与当前实际列映射：
            # 若顺序表与映射键不一致，使用实际识别到的选项，避免后续 Categorical 产生 NaN 选项行。
            option_order = [opt for opt in option_order if opt in option_to_col]
            if not option_order:
                option_order = list(option_to_col.keys())
            ...
            # 避免将未知标签强制转为 NaN；使用排序键排序而不改写原值
            order_map = {opt: idx for idx, opt in enumerate(option_order)}
            final_table['_sort_key'] = final_table['选项内容'].map(order_map).fillna(10**9)
            final_table = final_table.sort_values('_sort_key').drop(columns=['_sort_key']).reset_index(drop=True)
            for p_type in all_player_types:
        ...
        if run_statistical_tests:
            ...
            for p_type in unique_player_types:
                ...
                for q_num in QUESTION_TYPES.get("多选", []):
                    q_num_str = str(q_num)
                    q_cols_multi = get_question_columns(df, q_num_str, include_colon=True)
        ...
        if results or within_sig_matrix_sheets_to_combine:
            with pd.ExcelWriter(
                ...
                if sorted_sig_matrices:
                    ...
                    # 先写“总体检验汇总表”（单选/评分/多选/填空等），避免只看到矩阵而误判“未分析”
                    if overall_significance_summary_list:
                        combined_sig_sheet.write(current_sig_row, 0, "组间总体检验汇总", first_col_format)
                        current_sig_row += 1
                        summary_df = pd.DataFrame(overall_significance_summary_list)
                        if not summary_df.empty:
                            summary_df = summary_df.sort_values(by=["题号", "题型"], kind="stable").reset_index(drop=True)
                            summary_cols = [
                                "题号",
                                "题型",
                                "题干",
                            ...
                            for col_idx, col_name in enumerate(summary_df.columns):
                                combined_sig_sheet.write(current_sig_row, col_idx, str(col_name), header_format)
                            current_sig_row += 1
                            ...
                            for _, row in summary_df.iterrows():
                                for col_idx, col_name in enumerate(summary_df.columns):
                                    ...
                                    if col_name == "总检验p值":
                                        if pd.notna(v):
                                            combined_sig_sheet.write_number(current_sig_row, col_idx, float(v), number_format)
                                        else:
                                            combined_sig_sheet.write(current_sig_row, col_idx, "", None)
                                    elif col_name == "总检验效应量":
                                        if pd.notna(v):
                                            combined_sig_sheet.write_number(current_sig_row, col_idx, float(v), number_format)
                                        else:
                                            combined_sig_sheet.write(current_sig_row, col_idx, "", None)
                                        ...
                                        if pd.notna(v):
                                            combined_sig_sheet.write_number(current_sig_row, col_idx, int(v), integer_format)
                                        else:
                                            combined_sig_sheet.write(current_sig_row, col_idx, "", None)
                                    else:
                                        combined_sig_sheet.write(current_sig_row, col_idx, str(v) if pd.notna(v) else "", None)
                                current_sig_row += 1
                            current_sig_row += 2
