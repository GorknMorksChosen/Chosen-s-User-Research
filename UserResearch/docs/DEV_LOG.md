...
**术语说明（当前生效）**：本日志中的「五剑客」「超级应用」等词汇若出现在旧日期条目中，均为历史语境记录，不代表当前命名；当前统一术语以 README「术语口径声明（当前生效）」为准。
...
**背景**：
- 用户实测中出现两类问题：
  1) 开放题被自动识别为单选/背景参考，导致在“分析目标题目（文本内容）”中不可选；
  2) 上传问卷大纲后，个别题目（如 Q16）未按大纲题型覆盖，实际根因是大纲 docx 中题号/题干/题型标签跨多行导致解析漏题。

**本轮落地内容**：
- **文本工具新增题型手动纠偏面板**（`问卷文本分析工具 v1.py`）：
  - 新增「题型识别手动调整（可选）」折叠区；
  - 支持按题号将题型覆盖为 `开放文本/单选/多选/矩阵/评分/NPS`；
  - 支持题目搜索（题号/关键词）；
  - 支持多选题目后批量应用/批量恢复自动识别；
  - 支持展示当前覆盖项并一键清空；
  - 上传新数据后自动清空旧覆盖，避免跨文件污染。
- **题号匹配口径统一**：
  - 对 `16` / `016` 等前导零题号做归一化比较，修复“已覆盖但选择器未生效”的隐性问题。
- **问卷大纲解析增强（问卷星 docx）**（`survey_tools/utils/outline_parser.py`）：
  - 新增“题号行后持续拼接，直到题型标签行”为止的通用多行合并逻辑；
  - 不再只依赖 2 行/3 行特例，兼容更长题干跨行场景；
  - 保留兜底：若未找到题型标签，不吞掉原始行。

**验证（用户样本）**：
- 数据：`d:/SUN用研运营/内部跑测/0331跑测/15人/15人跑测问卷_新人_文本.xlsx`
- 大纲：`d:/SUN用研运营/内部跑测/0331跑测/15人/【SUN 0330】15人跑测问卷.docx`
- 关键验证结论：
  - 修复前：`parsed_outline` 中缺失 Q16，Q16 未被大纲覆盖，仍可能识别为单选；
  - 修复后：Q16 可被大纲解析并映射为开放文本；手动覆盖后可在“分析目标题目（文本内容）”正常出现。

---

### game_analyst：数据体检离群值检测 TypeError 修复
...
**问题**：
- 执行“全量数据体检”时在离群值检测处报错：
  - `TypeError: can only concatenate str (not "int") to str`
- 根因：`fillna` 时使用了原始 `self.data[features].mean()`，对象列（含字符串）参与均值计算触发类型错误。

**修复**：
- 改为先整体数值化，再基于数值化结果求列均值并填补：
  - `numeric_df = self.data[features].apply(pd.to_numeric, errors='coerce')`
  - `numeric_df = numeric_df.fillna(numeric_df.mean())`

**验证**：
- `ReadLints`：`survey_tools/core/advanced_modeling.py` 无新增 linter 问题。
...
**本轮落地内容**：
- 新建专项 Streamlit 工具 `player_profiling_tags.py`，用于“问卷玩家画像多维打标签”；
- 上传链路支持 `.xlsx/.csv`，并在 Windows 下启用事件循环策略修复，规避 `Event loop is closed`；
- 与现有项目口径对齐：
  - 读取后先尝试 `normalize_wjx_headers` 表头规范化；
  - 列选择显示复用 `parse_columns_for_questions` 的题号识别。
- 打标链路：
  - 按“【题头】：回答”组装结构化输入，过滤 NaN/空值/跳过值；
  - Prompt 强约束“只能从预设标签库中选 1-3 个标签”并要求 JSON 输出；
  - 使用 `asyncio + aiohttp` 并发请求（并发上限可调）+ 指数退避重试；
  - 结果回填 `AI画像标签` / `AI打标理由`，并支持 Excel 下载。
- 已加增强能力（用户确认后落地）：
  - 失败样本重跑：仅重跑 `AI画像标签` 含 `待人工复核` 的行；
  - 标签统计看板：标签命中人数/占比 + Top10 标签组合；
  - 失败样本清单导出：附带人工复核模板列（人工复核标签/理由/是否确认/复核人/复核时间）；
  - 人工复核回写（可选折叠区）：仅回写 `是否确认=确认` 行，写入 `最终画像标签` / `最终打标理由` / `最终标签来源`，不覆盖 AI 原列；
  - 交付口径导出开关：导出时可优先最终列，自动生成“交付口径”字段并在最终缺失时回退 AI。

**本轮业务结论（产品侧）**：
- 当前功能先收敛，不继续叠加高复杂度交互；
- 复核闭环保留为“可选折叠区”，默认流程保持轻量，不干扰常规用研打标流程。

**下一步优化建议（仅建议，未在本轮实现）**：
1. 增加“复核清单导入校验报告”（缺列/越界行号/空标签明细），提升批量回写可解释性；
2. 增加“复核结果回滚”能力（按最近一次回写批次回退）；
3. 若专项工具稳定 1-2 轮，再评估并入 `tool_registry.py` 与 `web_tools_launcher.py`。
